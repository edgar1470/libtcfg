...
tcfg_config_data_t *tcfg_get_section(const char *config_data, const char *section_name)
{
        ...
        pstr = strstr(config_data, section_name);
        if (pstr == NULL) {
                log_error("can't find the section [%s]\n", section_name);
                return NULL;
        }
        setLen = strlen(section_name);
        pstr += setLen;
        return pstr;
}
...
tcfg_config_data_t *tcfg_get_config_data(char *filename)
{
        ...
        if (filesize > TCFG_CONFIG_FILE_MAX_SIZE) {
                log_error("config file size is > %d\n", TCFG_CONFIG_FILE_MAX_SIZE);
                fclose(stream);
                return NULL;
        }
        rewind(stream);
